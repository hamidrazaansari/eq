  ...
  return (
    <div className="container">
          ...
          return (
            <div key={box.id} className={`box ${boxClass} ${direction}`}>
              {box.type === "image" ?
                ...
                :
                <video src={box.image} autoPlay muted loop ></video>
              }
            </div>
          );
        })}
      ...
      <div className="dots-container">
        {[...Array(3)].map((_, index) => {
          let dotIndex = (currentBox + index) % boxes.length; // Show only 3 dots dynamically

          return (
            <span
              key={dotIndex}
              className={`dot ${currentBox === dotIndex ? 'active' : ''}`}
